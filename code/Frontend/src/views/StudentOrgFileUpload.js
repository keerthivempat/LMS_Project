...
const StudentOrgFileUpload = ({ organizationId, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/api/v1/admin/invite-students-to-org-file/${organizationId}`, {
        method: "POST",
        body: formData,
        headers: {
        ...
        {results && (
          <div className="mt-4 p-4 bg-green-50 rounded-lg border border-green-200">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="flex justify-between bg-white p-2 rounded">
                <span className="text-brown/80">Already in organization:</span>
              ...
              <div className="flex justify-between bg-white p-2 rounded">
                <span className="text-brown/80">New accounts:</span>
                <span className="font-medium text-blue-500">{results.accountsCreated?.length || 0}</span>
              </div>
            </div>
            ...
            {results.accountsCreated && results.accountsCreated.length > 0 && (
              <div className="mt-3">
                <p className="text-sm font-medium text-brown/80 mb-1">New accounts created:</p>
                <div className="text-sm text-blue-600 bg-white p-2 rounded max-h-24 overflow-y-auto">
                  {results.accountsCreated.map((user, i) => (
            ...
            {results.alreadyInOrg && results.alreadyInOrg.length > 0 && (
              <div className="mt-3">
                <p className="text-sm font-medium text-brown/80 mb-1">Already in organization:</p>
                <div className="text-sm text-yellow bg-white p-2 rounded max-h-24 overflow-y-auto">
                  {results.alreadyInOrg.map((student, i) => (
                    <div key={i} className="mb-1 last:mb-0">
...
export default StudentOrgFileUpload; 